...
function Footer({ name, email, phoneNum, location }) {
  return (
    <footer className="w-full bg-white pt-10 pb-8 border-t border-gray-200">
      <div className="max-w-[1276px] mx-auto px-4 sm:px-6 lg:px-8">
        {/* Top Section - Logo, Navigation, Social */}
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
          {/* Logo */}
          <div className="font-manrope font-semibold text-[18px] sm:text-[20px] md:text-[22px] text-[#5700FF]">
            {name}
          </div>
          ...
          <div className="w-full md:w-auto">
            <ul className="flex flex-wrap justify-center gap-4 sm:gap-6 md:gap-8 font-manrope font-medium text-[16px] text-gray-700">
              {['Home', 'About Me', 'Portfolio', 'Contact Me'].map((item) => (
                <li key={item} className="hover:text-[#5700FF] transition-colors duration-200">
                  <a href="#" className="py-1 block">{item}</a>
                </li>
              ))}
            </ul>            
          </div>
          ...
          <div className="flex justify-center items-center gap-4">
            {[
              ...
              <a 
                key={index}
                href="#"
                className="w-10 h-10 rounded-full flex items-center justify-center bg-gray-100 hover:bg-[#5700FF] transition-all duration-300 group"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-[742px] mx-auto py-6 border-t border-gray-200 mt-4">
          {[
            ...
            >
              ...
              <span className="font-manrope text-gray-700 text-[16px]">{item.text}</span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 text-center">
          <p className="font-manrope text-[#808080] text-[14px]">
