...
function FaqComp({ question, answer }) {
  ...
  return (
    <div className={`w-full md:w-[90%] lg:w-[757px] bg-[#EEEBE5] rounded-lg overflow-hidden mb-4 transition-all duration-300 ${isOpen ? 'shadow-lg' : 'shadow'}`}>
      <div 
        className="flex justify-between items-center p-4 md:p-6 cursor-pointer hover:bg-[#e6e2d9] transition-colors"
        onClick={toggleIsOpen}
      >
        <p className='text-base md:text-lg lg:text-xl font-medium font-manrope text-[#333333] flex-grow pr-4'>
          {question}
        </p>
      ...
      {isOpen && (
        <div className="overflow-hidden">
          <div className="animate-royalAppear bg-[#f8f6f0] border-t border-[#e0dccf] p-4 md:p-6">
            <div className="relative">
              ...
              {/* Answer text */}
              <p className='text-sm md:text-base font-manrope text-[#4D4D4D] relative z-10 pt-1 pr-8'>
                {answer}
              </p>
            </div>
          </div>
...
function FAQSection({ faqs }) {
  return (
    <div className="w-full py-10 px-4 sm:px-6 lg:px-8 bg-[#faf9f7]">
